...
// ========================================================== //
const (
	shmName      = "Global\\MySharedMemory"
	shmSize      = 8192 // Adjust size as needed
	shmMutexName = "Global\\MySharedMemoryMutex"
)

func (s *System) initializeMutex() error {
	// Try to open an existing named mutex
	mutexHandle, err := windows.OpenMutex(0x001F0001, false, windows.StringToUTF16Ptr(shmMutexName))
	if err == nil {
		// Mutex exists, use the existing handle
		s.mutexHandle = mutexHandle
		return nil
	}

	// Mutex does not exist, create a new one
	mutexHandle, err = windows.CreateMutex(nil, false, windows.StringToUTF16Ptr(shmMutexName))
	if err != nil {
		return fmt.Errorf("failed to create named mutex: %w", err)
	}
	s.mutexHandle = mutexHandle
	return nil
}
...
func (s *System) releaseMutex() {
	windows.ReleaseMutex(s.mutexHandle)
}

// Wait to acquire the named mutex
func (s *System) acquireMutex() error {
	result, err := windows.WaitForSingleObject(s.mutexHandle, windows.INFINITE)
	if err != nil {
		return fmt.Errorf("failed to wait for mutex: %w", err)
	}
	if result == windows.WAIT_ABANDONED {
		return fmt.Errorf("mutex was abandoned")
	}
	return nil
}

func (s *System) createSharedMemory() error {
	fmt.Println("Creating shared memory...")
	...
	if err != nil {
		windows.CloseHandle(s.shmHandle)
		return err
	}
	s.shmAddr = addr
	fmt.Println("Created shared memory")

	return nil
}

// Function to clean up shared memory
func (s *System) cleanupSharedMemory() {
	if s.shmAddr != 0 {
		windows.UnmapViewOfFile(s.shmAddr)
	}
	if s.shmHandle != 0 {
		windows.CloseHandle(s.shmHandle)
	}
...
func (s *System) writeToSharedMemory(data string) {
	copy((*[shmSize]byte)(unsafe.Pointer(s.shmAddr))[:], data)
}

func writeSharedMemory(shmAddr uintptr, data []byte) {
	copy((*[1 << 30]byte)(unsafe.Pointer(shmAddr))[:], data)
}

// Function to read data from shared memory
func (s *System) readFromSharedMemory() string {
	data := (*[shmSize]byte)(unsafe.Pointer(s.shmAddr))[:]
	return string(data)
}

func clearSharedMemory(shmAddr uintptr) {
	// Implement shared memory clearing logic
	size := shmSize // Example size, adjust as needed
	mem := make([]byte, size)
	writeSharedMemory(shmAddr, mem)
}
...
func (s *System) fight() (reload bool) {
	// Initialize named mutex
	if err := s.initializeMutex(); err != nil {
		fmt.Println("Failed to initialize mutex:", err)
		return
	}
	// Create and attach shared memory
	err := s.createSharedMemory()
	if err != nil {
		panic(err)
	}
	defer s.cleanupSharedMemory()

	// Setup signal handling to clean up shared memory on exit
	c := make(chan os.Signal, 1)
	signal.Notify(c, os.Interrupt, syscall.SIGTERM)
	go func() {
		<-c
		s.cleanupSharedMemory()
		os.Exit(1)
	}()

	shmInitMsg := "Initialize"
	s.writeToSharedMemory(shmInitMsg)

	fmt.Println("Data written to shared memory.")

	// Read data from shared memory to verify
	readData := s.readFromSharedMemory()
	fmt.Printf("Data read from shared memory: %s\n", readData)
	clearSharedMemory(s.shmAddr)
	...
	for !s.endMatch {
		...
		if s.tickFrame() {
			// Send game states to model for preprocessing
			var gameStates []GameState

			gameState0 := s.GetGameState(0)
			gameState1 := s.GetGameState(1)

			// Append both states to the slice
			gameStates = append(gameStates, gameState0, gameState1)

			dataWrapper := struct {
				IsRoundOver bool        `json:"isRoundOver"`
				States      []GameState `json:"states"`
			}{
				IsRoundOver: s.matchOver(), // Use the appropriate function or flag to determine if the round has ended
				States:      gameStates,
			}

			// Marshal the combined states back into JSON
			combinedJSON, err := json.Marshal(dataWrapper)
			if err != nil {
				fmt.Println("Error marshaling combined GameStates:", err)
				return
			}

			// Acquire the mutex before writing to shared memory
			if err := s.acquireMutex(); err != nil {
				fmt.Println("Error acquiring mutex:", err)
				return
			}

			// Clear and write the combined game state to shared memory
			clearSharedMemory(s.shmAddr)
			writeSharedMemory(s.shmAddr, combinedJSON)
			// fmt.Println("Data written to shared memory.")

			// Optional: read back to confirm (debug)
			// readData := s.readFromSharedMemory()
			// fmt.Printf("Data read from shared memory: %s\n", readData)

			// Release the mutex after writing is complete
			s.releaseMutex()
		}
