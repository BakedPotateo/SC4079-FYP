...
def run_agent(agent_id, hwnd, action_space, action_to_key, device, state_vector, gamma, episode, start_time):
    ...
    while True:
        ...
        logits = logits + torch.tensor(skew[agent_id], device=logits.device, dtype=logits.dtype)  # Add skew matrix based on agent_id
        ...
        if np.random.rand() < epsilon:
                action_idx = np.random.choice(len(action_space.actions))  # Random action from non-movement actions
        else:
            action_idx = np.random.choice(len(action_space.actions), p=action_probs_np)  # Probabilistic action from non-movement actions
...
def save_model(model, optimizer, agent_id):
    ...
    filename = f"actor_critic_model_{agent_id}.pth"
    torch.save({
...
def load_model(model, optimizer, filename="actor_critic_model_0.pth"):
...
if __name__ == "__main__":
    ...
    if hwnd:
        if mode == 0:
            ...
            load_model(model, optimizer, filename="actor_critic_model_1.pth")
